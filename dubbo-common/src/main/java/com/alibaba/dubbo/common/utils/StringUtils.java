...
public final class StringUtils {
	...
	public static String toQueryString(Map<String, String> ps) {
        return mapToString(ps, "&", "=");
	}

    /**
     * 将map中的数据拼装成字符串
     * @param map   需要拼接的数据
     * @param itemSeparator 每条数据间的分割符
     * @param kvSeparator   键与值的分割符
     * @return  拼装后的字符串
     */
    public static String mapToString(Map<String, String> map, String itemSeparator, String kvSeparator) {
        StringBuilder buf = new StringBuilder();
    ...
    /**
     * 根据指定的开始字符与结束字符取出之间的内容
     * <p>
     *     getStringBySymbol("a[1]fd[b]2[334]", "[", "]")
     * </p>
     * @param cont  需处理的数据
     * @param start 开始字符串
     * @param end   终止字符串
     * @return  取出的数据集合
     */
    public static List<String> getStringBySymbol(String cont, String start, String end) {
        ...
        int startPos = 0;
        int endPos = 0;
        while (!StringUtils.isBlank(cont)) {
            startPos = cont.indexOf(start);
            endPos = cont.indexOf(end);
            if (startPos < 1 || endPos < 1) {
                // 取完结束
                break;
            }
            result.add(cont.substring(startPos, endPos + 1));
            cont = cont.substring(endPos+1);
        }
        return result;
    }
