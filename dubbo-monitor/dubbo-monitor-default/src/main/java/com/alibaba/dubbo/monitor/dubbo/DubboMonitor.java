...
public class DubboMonitor implements Monitor {
    ...
    public void send() {
        ...
        for (Map.Entry<Statistics, AtomicReference<long[]>> entry : statisticsMap.entrySet()) {
            ...
            URL url = statistics.getUrl()
                    ...
                    .addParameterString(collectThreadPool(statistics.getUrl().hasParameter(CONSUMER)));
     ...
     * 采集线程池中的数据，以供发送到监控模块
     * 目前只采集服务端的线程池数据
     * @param isServer 是否服务端数据
     * @return  线程池信息
     */
    private String collectThreadPool(boolean isServer) {
        String thread = ""; // 当前服务已经使用线程数
        String side = isServer ? Constants.PROVIDER_THREAD_CHANNEL_HANDLER : Constants.CONSUMER_THREAD_CHANNEL_HANDLER;
        if (RpcContext.getReadOnlyContext().containsKey(side)) {
